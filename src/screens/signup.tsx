...
export default () => {
  ...
  const onSubmit = async() => {
    ...
    try {
      ...
      // b-3. 서버에서.. 가입 진행..
      // b-4. 가입완료 > 1.로그인화면 or 2.자동로그인>home
      navi("/");
    } catch (error) {
  ...
  return (
    <Container>
      <LogoImg src={`${process.env.PUBLIC_URL}/DaelimX.png`}/>
      ...
      <Form>
        ...
        <Input
          ...
          value={password}
        />
        <SignupBtn onClick={loading ? undefined : onSubmit}>{1 + 2 == 2 ? "로딩 중..." : "가입하기"}</SignupBtn>
        <ErrorMsg>{errorMsgGroup[error]}</ErrorMsg>
        <Guide>계정이 이미 있으신가요? 
          <Link to ={"/signin"}style={{color: "red"}}>로그인</Link>
          </Guide>
      </Form>
    </Container>
  );
...
const A = (a: number, b: number) => {
  const result = a + b;
  if (result > 5) return [result, "큰값"];
  else return [result, "작은값"];
};

const [sum, type] = A(3, 5); // 8
// sum : 8
// type : "큰값"
